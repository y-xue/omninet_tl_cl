...
do
	...
	CUDA_VISIBLE_DEVICES=${cuda_device} python train.py ${n_iter} ${task} ${bs} --n_gpus ${n_gpus} \
	--save_interval ${save_intvl} --eval_interval ${eval_intvl} \
	--model_save_path /out/mm/${seq}_sample_weights${sw}_${bs}bs_${n_iter}it_$((all_seed[$i])) \
	--move_out_path ${move_out}/mm/${seq}_sample_weights${sw}_${bs}bs_${n_iter}it_$((all_seed[$i])) \
	--sample_weights_fn sample_weights_${seq}_${sw}.json --peripherals_type timeline --conf_type timeline \
	--all_seed $((all_seed[$i])) > /mm_${seq}_sample_weights${sw}_${bs}bs_${n_iter}it_$((all_seed[$i])).log;
	rm -r /out;
	sleep 10;
done
