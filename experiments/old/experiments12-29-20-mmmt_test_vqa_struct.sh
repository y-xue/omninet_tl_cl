...
test(){
	...
	--inject_at_logits > ${cdn}/out/omninet/${model_name}_predictions.log;
}
...
test1(){
	...
	--fusion_attn_type ${fusion_attn_type} > ${cdn}/out/omninet/${model_name}_predictions.log;
}
...
test3(){
	...
	--convex_gate > ${cdn}/out/omninet/${model_name}_predictions.log;
}
