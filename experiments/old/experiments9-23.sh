...
do
	...
	--model_save_path /out/mm_rewarding/${seq}_prevent_overfitting_sample_weights${sw}_${bs}bs_${n_iter}it_$((all_seed[$i])) \
	--move_out_path ${move_out}/mm_rewarding/${seq}_prevent_overfitting_sample_weights${sw}_${bs}bs_${n_iter}it_$((all_seed[$i])) \
	...
	--all_seed $((all_seed[$i])) --rewarding --overfitting_threshold ${ot}  > /mm_rewarding_${seq}_prevent_overfitting_sample_weights${sw}_${bs}bs_${n_iter}it_$((all_seed[$i])).log;
	rm -r /out/mm_rewarding/${seq}_prevent_overfitting_sample_weights${sw}_${bs}bs_${n_iter}it_$((all_seed[$i]));
	sleep 10;
